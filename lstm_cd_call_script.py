...
industry = "Consumer Discretionary"  # Change to Health Care
...
def prepare_lstm_data(data, sequence_length=60, scaler=None):
    ...
    for ticker in data["Ticker"].unique():
        ...
        for i in range(len(ticker_data) - sequence_length):
            sequence = ticker_data.iloc[i:i+sequence_length][numerical_cols].values
...
joblib.dump(scaler, f"{industry}_scaler.pkl")
print(f"Model and scaler for {industry} saved.")
...
# Save results
results_df.to_csv(f"{industry}_predictions.csv", index=False)
print(f"Predictions saved to '{industry}_predictions.csv'")
